...
void timerSubscribe() {
    client.subscribe(MQTTid "/1/ton", MQTTsubQos);
    client.subscribe(MQTTid "/2/ton", MQTTsubQos);
    client.subscribe(MQTTid "/3/ton", MQTTsubQos);
    client.subscribe(MQTTid "/4/ton", MQTTsubQos);
    client.subscribe(MQTTid "/1/toff", MQTTsubQos);
    client.subscribe(MQTTid "/2/toff", MQTTsubQos);
    client.subscribe(MQTTid "/3/toff", MQTTsubQos);
    client.subscribe(MQTTid "/4/toff", MQTTsubQos);
}
...
void mqttDataCb(char* topic, byte* payload, unsigned int length) {
  ...
  if (config) {
    ...
    if (!strcmp(topic, MQTTid "/ssid"))
      EPSW(SETUPssidAddr, message);
    else if (!strcmp(topic, MQTTid "/psw"))
      EPSW(SETUPpswAddr, message);

    if (!strcmp(topic, MQTTid "/ssid") || !strcmp(topic, MQTTid "/psw"))
      ...
      switch(events[currentEvent]) {
        case EVNT_CONN: {
          if (!config) {
            ...
            client.subscribe(MQTTid "/1/on", MQTTsubQos);
            client.subscribe(MQTTid "/2/on", MQTTsubQos);
            client.subscribe(MQTTid "/3/on", MQTTsubQos);
            client.subscribe(MQTTid "/4/on", MQTTsubQos);
            
          } else {
        
            client.subscribe(MQTTid "/ssid", MQTTsubQos);
            client.subscribe(MQTTid "/psw", MQTTsubQos);
